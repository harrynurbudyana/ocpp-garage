...
import { addDriver, checkInvitationLink } from "@/services/drivers";
import { addOperator } from "@/services/operators";
...
const onSubmit = () => {
  ...
  let role = data.value.role;
  delete data.value.garageId;
  delete data.value.confirm_password;
  let sender = role === Role.resident ? addDriver : addOperator;
  sender({ garageId, data: data.value })
    .then(() => {
